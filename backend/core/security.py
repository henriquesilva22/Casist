...
def create_access_token(data: Dict[str, Any], expires_delta: Optional[timedelta] = None) -> str:
  to_encode = data.copy(); expire = datetime.utcnow() + (expires_delta or timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)); to_encode.update({'exp': expire}); return jwt.encode(to_encode, SECRET_KEY, algorithm=ALGORITHM)
def get_current_user(token: Optional[str] = Depends(oauth2_scheme)) -> Dict[str, Any]:
  ...
  try:
    ...
    if token.startswith('local:') and token[6:].strip():
      return {'username': token[6:].strip()}
    return {'username': 'guest'}
